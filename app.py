...
"""
    ...
    if gemini_model:
        try:
            ...
            print("Gemini API error in simple analysis:", e)
...
# GEMINI SETUP
# ==========================================
GEMINI_KEY = os.environ.get("GEMINI_API_KEY", "AIzaSyB9AGCCZxR7-bjS69OYHgd8R40RTIu--Dk")
...
def root_ai(user, pred, language='en'):
    ...
    prompt = f"""
You are an expert AI clinical health and lifestyle analyst, but with a very warm, encouraging, and user-friendly tone. Provide a realistic, easy-to-understand, and ultra-concise assessment based on the user's data.
...
- Steps Walked: {user.get('steps_walked', 'N/A')}
- Exercise: {user.get('exercise_minutes', 'N/A')} mins
- Water Intake: {user.get('water_intake', 'N/A')} L
- Diet: {user.get('calories_intake', 'N/A')} kcal, {user.get('protein_intake', 'N/A')}g protein
...
2. Be extremely concise. Use short bullet points only. Zero fluff, no intro/outro paragraphs.
3. Base actionable advice on established health protocols but explain them simply.

Include EXACTLY these 5 sections (use exactly ## for headings):

## 🩺 Your Health Baselines
(2-3 concise bullet points explaining what your lifestyle data means)

## ⚡ Your Actionable Activity Plan
(Concise, realistic exercise steps based on the ML baseline)

## 🍎 Simple Nutrition Tweaks
(Concise, easy-to-follow dietary adjustments)

## 🧠 Mind & Sleep Recovery
(Concise, relaxing tips for stress management and sleep)

## 💡 Key Focus for Today
(1-2 easy things to focus on today)
"""
    if language == 'hi':
...
## ✨ Skin Glow Analysis
(2-3 concise, realistic tips on how their stress, sleep, and hydration are affecting their skin, and how to improve it)
...
We couldn't connect to the AI coach. However, based on your data:
- **Sleep**: {user.get("sleep_hours", 8)} hours recorded. Aim for 7-9 hours.
- **Stress**: {user.get("stress_level", 0)}/10. Try integrating 10 mins of mindfulness.
- **Activity**: {user.get("steps_walked", 0)} steps. 10k steps daily is a good target.
- **Hydration**: {user.get("water_intake", 0)}L. Aim for at least 2.5L.

### 📋 Basic Plan
- Focus on consistency with your meals and macros.
- Try a simple 30-min walk or light cardio today.
...
def predict():
    ...
    try:
        ...
        try:
            ai_markdown = root_ai(data, prediction, language=lang)
        except Exception as e:
            print("Gemini Error:", e)
            ai_markdown = fallback(data)
...
def predict_hair_skin():
    ...
    try:
        ...
        try:
            ...
            print("Gemini Error:", e)
            ai_markdown = "## ⚠️ AI Service Unavailable\n\nCould not fetch your hair & skin report at this time."
...
def analyze_detail():
    ...
    try:
        ...
        try:
            ...
            print("Gemini Error:", e)
            ai_markdown = "## ⚠️ Analysis Unavailable\nCould not fetch detailed analysis at this time."
